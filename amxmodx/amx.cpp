...
{
    // KTP: Check cvar first (cheap float compare) before doing plugin lookup
    if (amxmodx_perflog->value > 0.0f)
    {
        CPluginMngr::CPlugin* perf_Plug = g_plugins.findPluginFast(amx);
        if (perf_Plug && (perf_Plug->isDebug() || (int)amxmodx_debug->value == 2))
        {
